...
def main():
    ...
    print(
        f"Generated input signal: {Usim.shape}, noise variance: {white_noise_variance}"
    )
    ...
    # Create DataFrame
    df_data = pd.DataFrame({"u": Usim, "y": Ytot}, index=Time)

    # Create IDData
    iddata = IDData(data=df_data, inputs=["u"], outputs=["y"], tsample=sampling_time)
    ...
    if mode == "FIXED":
        ...
        print(
            f"Target orders - na: {na_ord}, nb: {nb_ord}, nc: {nc_ord}, delays: {theta}"
        )
        ...
        for i, (name, options) in enumerate(algorithms):
            ...
            try:
                ...
                print(
                    f"  - Target orders: na={na_ord[0]}, nb={nb_ord[0][0]}, nc={nc_ord[0]}"
                )
                ...
                reasonable_nk = min(
                    max_delay, 2
                )  # Much smaller delay for available data
                ...
                # Debug output
                print(
                    f"  - Debug: na={config.na}, nb={config.nb}, nc={config.nc}, nk={config.nk}"
                )
